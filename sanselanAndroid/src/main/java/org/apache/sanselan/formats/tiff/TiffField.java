...
public class TiffField implements TiffConstants
{
	...
	private static TagInfo getTag(int directoryType, int tag)
	{
		Object key = Integer.valueOf(tag);
		...
		return getTag(directoryType, tag, possibleMatches);
	}
	...
	private static Map makeTagMap(TagInfo tags[],
								  boolean ignoreDuplicates, String name)
	{
		...
		{
			TagInfo tag = tags[i];
			Object key = Integer.valueOf(tag.tag);
