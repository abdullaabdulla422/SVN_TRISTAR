		...
		BinaryConstants
{
			...
			throws ImageWriteException
	{
		...
		for (int i = 0; i < directories.size(); i++)
		{
			...
			Integer key = Integer.valueOf(dirType);
				...
				if (directoryIndices.contains(key))
					throw new ImageWriteException(
							"More than one directory with index: " + dirType
									+ ".");
				directoryIndices.add(Integer.valueOf(dirType));
			...
			for (int j = 0; j < fields.size(); j++)
			{
				...
				Integer fieldKey = Integer.valueOf(field.tag);
		...
		TiffOutputDirectory rootDirectory = (TiffOutputDirectory) directoryTypeMap
				.get(Integer.valueOf(DIRECTORY_TYPE_ROOT));
