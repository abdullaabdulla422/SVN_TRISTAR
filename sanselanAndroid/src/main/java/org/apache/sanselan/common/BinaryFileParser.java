...
public class BinaryFileParser extends BinaryFileFunctions
{
	...
	protected final int convertByteArrayToInt(byte bytes[])
	{
		return convertByteArrayToInt("ValueOffset", bytes, byteOrder);
	}
	...
	public final int read4Bytes(String name, InputStream is)
			throws ImageReadException, IOException
	{
		return read4Bytes(name, is, "Not a Valid TIFF File", byteOrder);
	}
	...
	{
		if ((bytes == null) || (bytes.length < org.apache.sanselan.formats.jpeg.JpegConstants.EXIF_IDENTIFIER_CODE.length))
			return false;

		for (int i = 0; i < org.apache.sanselan.formats.jpeg.JpegConstants.EXIF_IDENTIFIER_CODE.length; i++)
			if (bytes[i] != org.apache.sanselan.formats.jpeg.JpegConstants.EXIF_IDENTIFIER_CODE[i])
				return false;
