...
public class BinaryInputStream extends InputStream implements BinaryConstants
{
	...
	protected final void readRandomBytes() throws
			IOException
	{
			...
			throws IOException
	{
		...
		while (length != total)
		{
			...
			if (skipped < 1)
				throw new IOException("Couldn't skip bytes" + " (" + skipped + ")");
			total += skipped;
		}
	}
	...
	public final byte readByte(String name)
			throws IOException
	{
		int result = is.read();
		...
		if ((result < 0))
		{
			...
			throw new IOException("Random Data");
		}
												...
												byte bytes[], int byteOrder)
	{
		...
		if (byteOrder == BYTE_ORDER_MOTOROLA) // motorola, big endian
			result = ((0xff & byte0) << 8) + ((0xff & byte1));
		else
			// intel, little endian
			result = ((0xff & byte1) << 8) + ((0xff & byte0));
			...
			throws ImageReadException, IOException
	{
		...
		boolean strict = true;
		return readByteArray(length, error, true);
	}

	public final byte[] readByteArray(int length, String error,
									  @SuppressWarnings("SameParameterValue") boolean strict) throws ImageReadException,
			IOException
	{
			...
			throws ImageReadException, IOException
	{
		...
		if (byteOrder == BYTE_ORDER_MOTOROLA) // motorola, big endian
			...
			// intel, little endian
			return ((0xff & byte1) << 8) + ((0xff & byte0));
	}
	...
	public final int findNull(byte src[], @SuppressWarnings("SameParameterValue") int start)
	{
