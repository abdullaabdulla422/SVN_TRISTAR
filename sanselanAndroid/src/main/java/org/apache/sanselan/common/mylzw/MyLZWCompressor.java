...
public class MyLZWCompressor
{
	...
	private void InitializeStringTable()
	{
		...
		for (codes = 0; codes < intial_entries_count; codes++)
		{
			if ((codes != clearCode) && (codes != eoiCode))
			{
				...
				map.put(key, Integer.valueOf(codes));
			}
		}
	}
	...
	private Object arrayToKey(byte b)
	{
	...
	private Object arrayToKey(byte bytes[], int start, int length)
	{
		return new ByteArray(bytes, start, length);
	}

	private void writeDataCode(MyBitOutputStream bos, int code)
			throws IOException
	{
		if (null != listener)
	...
	private void writeCode(MyBitOutputStream bos, int code)
			throws IOException
	{
	...
	private int codeFromString(byte bytes[], int start, int length)
			throws IOException
	{
		Object key = arrayToKey(bytes, start, length);
	...
	private boolean addTableEntry(MyBitOutputStream bos, byte bytes[],
	...
	private boolean addTableEntry(MyBitOutputStream bos, Object key)
			throws IOException
	{
